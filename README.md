...
```js
import { LiteLru, Sieve, S3Fifo, WTinyLfu, Slru, TwoQ, Arc } from '@zakkster/lite-lru';
...
- **One `LiteCache<K,V>` surface** -- all seven members expose exactly `get` / `put` / `has` / `peek` / `delete` / `clear`, plus `size` and `capacity`. The recency/lazy-promotion/admission/adaptive difference is INTERNAL. Types ship in [`Lru.d.ts`](./Lru.d.ts); the interface is the type-checked contract that makes the one-line swap safe.
...
All seven classes implement `LiteCache<K,V>`. Every method is O(1) (amortized on the default `Map` backing; see [construction options](#construction-options)).
...
- **A stale-TTL `get` is a MISS and an EVICTION.** It counts one miss and reaps the expired entry in place (one eviction) -- consistent across all seven members.
...
| --------- | --------- | ------------------------------------------------------------ |
| `VERSION` | `'1.9.1'` | Package version string (in lock-step with `package.json` and `llms.txt`). |
