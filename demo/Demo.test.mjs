...
const CTORS = { LiteLru, Sieve, S3Fifo, WTinyLfu, Slru, TwoQ, Arc, Lirs, Lfu, ClockPro, LruK, Mq, Car };
...
test('every engine member has a renderer (no member silently unrendered)', () => {
    ...
    assert.equal(MEMBER_DEFS.length, 13);
});

test('assertion 1: rendered model deep-equals dump() over >= 2000 ops, all 13 (0 shadow fields)', () => {
    const cap = 32;
...
test('assertion 2: live hits/misses == from-scratch replay, 0 divergences over >= 2000 ops, all 13', () => {
    const cap = 32;
...
test('gap: degenerate capacities 1..4 -- all 13 members render without throwing, model==dump', () => {
...
test('gap: all three trace kinds (zipf/loop/scan) exercise assertion 1 for all 13 members', () => {
    const cap = 24;
    const kinds = {
...
test('gap: renderer teeth across ALL 13 members -- dropping or inventing ANY declared field fails', () => {
    const cap = 20;
...
test('gap: keys:"int" backing smoke -- renderer model still deep-equals dump() for all 13 members', () => {
    const cap = 12;
